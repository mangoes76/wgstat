...
SCRIPT_URL="https://raw.githubusercontent.com/mangoes76/$SCRIPT_NAME/refs/heads/main/$SCRIPT_NAME.sh"
...
get_docker_container() {
  while true; do
    read -p "Enter Docker container name (type '.exit' to abort): " container_name

    # Abort install if user quits
    if [[ "$container_name" == ".exit" ]]; then
      echo "Installation aborted by user."
      rm -f "$SCRIPT_PATH" # Clean up the downloaded file
      exit 1
    fi

    # Ensure a valid container name is entered
    if [[ ! "$container_name" =~ ^[a-zA-Z0-9_.-]+$ ]]; then
      echo "Invalid Docker container name entered."
      continue
    fi    

    echo "Verifying Docker container '$container_name'..."
    if docker exec "$container_name" wg show > /dev/null 2>&1; then
      echo "Successfully connected to WireGuard in container '$container_name'."
      exec_cmd="$SCRIPT_PATH -c $container_name"
      break
    else
      echo "Error: Could not execute 'wg show' in container '$container_name'."
